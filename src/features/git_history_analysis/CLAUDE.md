...
- **New package** (T-NEW-PACKAGE, +25): Created within last 7 days
- **Malicious diff** (T-MALICIOUS-DIFF, +55): Latest commit introduces network-related code (curl, wget, nc, socat) where none existed in prior commits — the Xeactor attack pattern
...
- `shared/aur_git.rs` — git clone/pull/log/diff operations
...
- T-FORCE-PUSH: Detect rewritten git history (not yet implemented)
- T-CLUSTER-UPLOAD: Cross-correlate with maintainer_analysis for coordinated multi-package attacks
- T-ORPHAN-RUSH: Detect rapid orphan-then-adopt cycles
